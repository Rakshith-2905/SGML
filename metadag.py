...
class TreeGraph(object):
    def __init__(self, hidden_dim, input_dim):
        ...
        self.lstmae_proto = LSTMAutoencoder(hidden_num=FLAGS.hidden_dim, input_num=FLAGS.hidden_dim,
                                      name='lstm_proto')
        self.lstmae_meta = LSTMAutoencoder(hidden_num=FLAGS.hidden_dim, input_num=FLAGS.hidden_dim,
                                            name='lstm_meta')    
    ...
    def model(self, inputs):
        ...
        for i, level in enumerate(self.graph_tree):
            ...
            if i == 0:
                ...
                task_embed_vec, task_emb_loss = self.lstmae_proto.model(updated_proto_graph_nodes)
                task_embed_vec = tf.identity(task_embed_vec, name='level_{}_graph_{}_embedding'.format(i, 0))
                proto_embed_loss.append(task_emb_loss)
                updated_proto_embeddings.append(task_embed_vec)
                ...
                for j, prev_level_updated_graph, prev_level_updated_embedding in zip(range(len(tree_embeddings[-1])), tree_graphs[-1], tree_embeddings[-1]):
                    ...
                    for k, graph in enumerate(level):
                        ...
                        meta_embed_vec, emb_loss = self.lstmae_meta.model(current_graph)
                        meta_embed_vec = tf.identity(meta_embed_vec, name='level_{}_meta_graph_{}_embedding'.format(i, k))
                        meta_embed_loss.append(emb_loss)
                ...
                for k, graph in enumerate(updated_proto_graphs):
                    task_embed_vec, task_emb_loss = self.lstmae_proto.model(graph)
                    task_embed_vec = tf.identity(task_embed_vec, name='level_{}_proto_graph_{}_embedding'.format(i, k))
                    proto_embed_loss.append(task_emb_loss)
                    updated_proto_embeddings.append(task_embed_vec)
            ...
            # print("\n***********************************************\n\n\n")
        proto_embed_loss = tf.reduce_mean(tf.stack(proto_embed_loss))
        meta_embed_loss = tf.reduce_mean(tf.stack(meta_embed_loss))
        return tree_graphs[-1][0], proto_embed_loss, meta_embed_loss
